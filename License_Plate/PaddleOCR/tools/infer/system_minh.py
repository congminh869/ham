...
def main(args):
    ...
    while True:
        if len(DATA_MQTT)!=0:
            ...
            image_decode = cv2.imdecode(np.fromstring(data, dtype=np.uint8), cv2.IMREAD_UNCHANGED)
            ...
            if box_kps_str != "":
                ...
                for box_kps in box_kpss:
                    ...
                    starttime = time.time()
                    if MODE_SYS: #If you combine the plate into 1 line, it will be considered as 1 word sometime 2 word
