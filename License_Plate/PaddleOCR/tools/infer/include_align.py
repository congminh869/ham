...
def calculate_missed_coord_corner(coordinate_dict):
    ...
    if index == 0:
        midpoint = np.add(coordinate_dict['top-right'], coordinate_dict['bottom-left']) / 2
        ...
        midpoint = np.add(coordinate_dict['top-left'], coordinate_dict['bottom-right']) / 2
        ...
        midpoint = np.add(coordinate_dict['bottom-left'], coordinate_dict['top-right']) / 2
...
def perspective_transform(image, source_points, type_lp, width, height): # type_lp = 1 -> biển dài, type_lp = 0 -> biển ngắn
    # if type_lp == 1:
    width = int(width)
    height = int(height)
...
def align_image(image, coordinate_dict):
    ...
    width = np.linalg.norm(np.array([top_left_point[0]-top_right_point[0], top_left_point[1]-top_right_point[1]]))
    height = np.linalg.norm(np.array([top_left_point[0]-bottom_left_point[0], top_left_point[1]-bottom_left_point[1]]))
    ...
    crop = perspective_transform(image, source_points, type_lp, width, height)
...
def mode_sys(dt_boxes, rec_res, img, args, count_plate):
    ...
    cv2.imwrite(
        os.path.join(save_plate, str(count_plate)+'.jpg'),
        draw_img[:, :, ::-1])
...
def check_format_plate(lp):
    ...
    return plate_format1.match(lp) is not None or plate_format2.match(lp) is not None or plate_format3.match(lp) is not None
...
def show_multi_cam(buffer_frames, buffer_imgs):
    ...
    for index in buffer_imgs:
        if len(buffer_imgs[str(index)]) != 0:
            ...
            # print(x1,y1,x2,y2)
            resized_image = cv2.resize(buffer_imgs[str(index)], (int(W_FRAME_SUB), int(H_FRAME_SUB))) 
