...
class DataImage:
    ...
    def __init__(self, _cid = -1, _type_id = -1, _count = -1 , _image = None):
        self.cid = _cid
...
class SHMReader:   
    ...
    def _ChecKCountWrite(self, memory_value):
        count = np.frombuffer(memory_value[:8], dtype='>u8')[0]
        if count > self.count_write:
            self.count_write = count
            return True
        return False
    ...
    def _ParserData2(self,memory_value):
        array_frame = []
        array_ret = []
        t_get = 0
        #Get image   
    ...
    def Read(self):
        ...
        if self.memory is not None:
            ...
            memory_value = self.memory.read(self.info_channel_shm.size_shm * self.buf_sz + 8)
            #print("Size N= ", len(memory_value))
            #if len(memory_value) < self.info_channel_shm.size_shm * self.buf_sz + 8:
                #print("Frame failed")
                #return -1, []
            t2 = time.time()
            #print("Time read memory = ",  t2 - t1)
            t1_p = time.time()
            #f_check = self._ChecKCountWrite(memory_value)
            #if f_check is True:
            #print(memory_value[:20])
    ...
    def Read2(self):
        ...
        if self.memory is not None:
            self.buf_sz = self.info_channel_shm.width_img * self.info_channel_shm.height_img * self.info_channel_shm.depth_img + 36
            memory_value = self.memory.read(self.info_channel_shm.size_shm * self.buf_sz + 8)
            arr = self._ParserData2(memory_value)
            t2_p = time.time()
            if len(arr) > 0:
                ret = 1
        return ret, arr              
