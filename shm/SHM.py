...
class SHMWriter:
    ...
    def __init__(self, _channel):
        ...
        self.size_package = self.info_channel_shm.width_img * self.info_channel_shm.height_img * self.info_channel_shm.depth_img + 4
        self.size_shm = self.info_channel_shm.size_shm * self.size_package + 4;
    ...
    def Init(self):
        self.memory = sysv_ipc.SharedMemory(self.info_channel_shm.key_shm, flags=sysv_ipc.IPC_CREAT, size=self.size_shm, mode=0o666)
            
    def _convert_DataImage_to_Bytes(self, frame):
        header = b'\x88\x88\x88\x88'
        
        info = struct.pack('>IIQ',frame.cid, frame.type_id, frame.count)
        encoded = header + info + frame.image.tobytes()
        return encoded
        
    def Write(self, frame):
        data = self._convert_DataImage_to_Bytes(frame)
        if self.memory is not None:
            if self.pos_package >=  self.info_channel_shm.size_shm:
                self.pos_package = 0
            
            self.memory.write(data, self.pos_package * self.info_channel_shm.size_shm + 4)
            data_pos = struct.pack('>I',self.pos_package)
            self.memory.write(data_pos, 0)
            self.pos_package+=1            
    ...
    def Read(self):
        ...
        if self.memory is not None:
            ...
            if pos < self.info_channel_shm.size_shm:
                if self.pos_pre < self.info_channel_shm.size_shm - 1:
                    if pos > self.pos_pre:
                        self.pos_pre = pos
                ...
                elif self.pos_pre == self.info_channel_shm.size_shm - 1:
                    if pos < self.pos_pre:
                        self.pos_pre = pos
                        self.buf_sz = self.info_channel_shm.width_img * self.info_channel_shm.height_img * self.info_channel_shm.depth_img + 36
                        memory_value = self.memory.read(self.buf_sz, offset = self.buf_sz * pos + 4)
